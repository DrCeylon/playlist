...
class JsonRpcEngineBridge(EngineBridge):
    ...
    def _dispatch(self, request: BridgeRequest) -> Iterator[dict[str, Any]]:
        ...
        if request.command == BridgeCommand.PLAN_SYNC:
            if self.backend is None or not hasattr(self.backend, "plan_sync"):
                raise BridgeError(BridgeErrorCode.NOT_CONFIGURED, "Backend de planification sync non configuré.")
            result = self.backend.plan_sync(request.params)
            yield BridgeResponse(id=request.id, ok=True, result=result).to_dict()
            return
        ...
        if request.command == BridgeCommand.IMPORT_REMOTE_PLAYLIST:
            if self.backend is None or not hasattr(self.backend, "import_remote_playlist"):
                raise BridgeError(BridgeErrorCode.NOT_CONFIGURED, "Backend d'import distant non configuré.")
            result = self.backend.import_remote_playlist(request.params)
            yield BridgeResponse(id=request.id, ok=True, result=result).to_dict()
            return

        if request.command == BridgeCommand.APPLY_SYNC:
            if self.backend is None or not hasattr(self.backend, "apply_sync"):
                raise BridgeError(BridgeErrorCode.NOT_CONFIGURED, "Backend d'application sync non configuré.")
            result = self.backend.apply_sync(request.params)
            yield BridgeResponse(id=request.id, ok=True, result=result).to_dict()
            return
